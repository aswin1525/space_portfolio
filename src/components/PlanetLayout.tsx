...
export default function PlanetLayout({ children, planetName, onReturn, colorTheme }: PlanetLayoutProps) {
    ...
    const themeColors: Record<string, { border: string; text: string; bg: string; glow: string }> = {
        teal: { border: "border-teal-500", text: "text-teal-400", bg: "bg-teal-900/20", glow: "shadow-teal-500/50" },
        purple: { border: "border-purple-500", text: "text-purple-400", bg: "bg-purple-900/20", glow: "shadow-purple-500/50" },
        blue: { border: "border-blue-500", text: "text-blue-400", bg: "bg-blue-900/20", glow: "shadow-blue-500/50" },
        orange: { border: "border-orange-500", text: "text-orange-400", bg: "bg-orange-900/20", glow: "shadow-orange-500/50" },
        green: { border: "border-green-500", text: "text-green-400", bg: "bg-green-900/20", glow: "shadow-green-500/50" },
        pink: { border: "border-pink-500", text: "text-pink-400", bg: "bg-pink-900/20", glow: "shadow-pink-500/50" },
    };

    const theme = themeColors[colorTheme] || themeColors.teal;

    return (
        <div className="relative w-full min-h-screen pt-20 pb-10 px-4 md:px-10 overflow-y-auto">
            {/* Background Planet Surface Texture (Subtle) */}
            <div className={`fixed inset-0 opacity-10 pointer-events-none bg-[radial-gradient(circle_at_center,_${theme.bg}_0%,_transparent_70%)]`} />
